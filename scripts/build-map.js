/* Build a compact SVG world map from Natural Earth GeoJSON.
   Robinson projection + Douglas-Peucker simplification.

   SOURCE: ne_10m_admin_0_countries_ukr.geojson — Natural Earth's Ukraine
   point-of-view edition. The default Natural Earth file draws *de facto*
   control, which puts Crimea inside Russia. Crimea is internationally
   recognised as Ukrainian territory (UN General Assembly Resolution
   68/262), which is the boundary an intergovernmental body must show.
   Only the 10m scale publishes point-of-view editions; the extra detail
   costs nothing here because TOLERANCE below governs the output density.

     curl -sLo /tmp/ne10m_ukr.geojson \
...
const SRC = process.env.NE_GEOJSON || '/tmp/ne10m_ukr.geojson';
const OUT = require('path').join(__dirname, '..', 'map-data.js');

const WIDTH = 1000;          // output viewBox width
const TOLERANCE = 0.6;       // simplification tolerance, output units
...
const out =
  `/* Auto-generated by scripts/build-map.js — do not hand-edit.\n` +
  `   Source: Natural Earth 10m admin-0, Ukraine point-of-view edition\n` +
  `   (public domain), which shows Crimea as Ukrainian territory per UN\n` +
  `   General Assembly Resolution 68/262. Robinson projection. */\n\n` +
  `'use strict';\n\n` +
  `const WORLD_MAP = {\n` +
