...
(function applyIcons() {
  ...
  const INPUT_ICONS = {
    ...
    fullName: 'user',
    nationality: 'flag',
    countryResidence: 'globe',
    email: 'mail',
    phone: 'phone',
    referenceNumber: 'hash',
    expertiseOther: 'sparkle',
  };

  Object.entries(INPUT_ICONS).forEach(([id, name]) => {
    const input = document.getElementById(id);
    if (!input || input.dataset.iconed) return;
    const wrap = document.createElement('div');
    wrap.className = 'input-wrap';
    input.parentNode.insertBefore(wrap, input);
    wrap.insertAdjacentHTML('afterbegin', icon(name, 'input-ico'));
    wrap.appendChild(input);
    input.classList.add('has-ico');
    input.dataset.iconed = '1';
  });

  // The languages chip-field and country search need the glyph inside their
  // own container rather than a new wrapper.
  const langField = document.getElementById('languageField');
  if (langField) {
    langField.insertAdjacentHTML('afterbegin', icon('translate', 'input-ico input-ico-static'));
    langField.classList.add('has-ico');
  }

  const search = document.querySelector('.picker-search');
  if (search) {
    const old = search.querySelector('.picker-search-icon');
    if (old) old.remove();
    search.insertAdjacentHTML('afterbegin', icon('eye', 'input-ico'));
  }

  /* --- badges on option cards --- */
  const OPTION_ICONS = {
    'Consultant (Technical / Advisory Assignments)': 'briefcase',
    'Peer reviewer (Proposal / Project Evaluation)': 'clipboard',
    'Researcher (research, implementation, training, and evaluation)': 'flask',
    'Individual Consultant': 'user',
    'Consulting Firm / Organization': 'building',
    'Short-term assignments (<3 months)': 'clock',
    'Medium-term (3-12 months)': 'hourglass',
    ...
    'Other (specify)': 'sparkle',
  };

  $$$('.option').forEach((label) => {
    const input = label.querySelector('input');
    if (!input || label.dataset.iconed) return;
    const name = OPTION_ICONS[input.value];
    if (!name) return;
    const body = label.querySelector('.option-body');
    if (!body) return;
    body.insertAdjacentHTML('beforebegin', `<span class="option-ico">${icon(name)}</span>`);
    label.dataset.iconed = '1';
  });

  /* --- a badge beside each step heading --- */
  const STEP_ICONS = {
    1: 'briefcase', 2: 'user', 3: 'doc', 4: 'shield', 5: 'globe', 6: 'pen',
    7: 'calendar', 8: 'euro', 9: 'history', 10: 'check', 11: 'layers',
  };

  Object.entries(STEP_ICONS).forEach(([step, name]) => {
    const section = document.querySelector(`.step[data-step="${step}"]`);
    if (!section) return;
    const title = section.querySelector('.step-title');
    if (!title || title.dataset.iconed) return;
    title.insertAdjacentHTML('beforebegin', `<span class="step-ico">${icon(name)}</span>`);
    title.dataset.iconed = '1';
  });

  /* --- upload zones --- */
  $$$('.dropzone-empty').forEach((zone) => {
    const old = zone.querySelector('.dropzone-icon');
    if (!old) return;
    old.insertAdjacentHTML('beforebegin', icon('upload', 'dropzone-icon'));
    old.remove();
  });

  /* --- labelled fields that aren't inputs --- */
  const LABEL_ICONS = [
    ['summary', 'pen'],
    ['dailyRate', 'clock'],
    ['hourlyRate', 'hourglass'],
  ];

  LABEL_ICONS.forEach(([id, name]) => {
    const label = document.querySelector(`label[for="${id}"]`);
    if (!label || label.dataset.iconed) return;
